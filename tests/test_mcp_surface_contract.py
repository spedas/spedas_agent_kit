...
def test_build_profile_snapshot_no_optional_analysis_tools_to_filter():
    """With the optional analysis layer removed the exclusion set is empty, so
    every live tool survives canonicalization and the snapshot documents the
    (now-empty) exclusion."""
    assert contract._ANALYSIS_TOOL_NAMES == frozenset()
    tools = [_tool_dump(name="browse_data_sources"), _tool_dump(name="transform_coordinates")]
    snap = contract.build_profile_snapshot("base", {}, tools, [], [], [])
    names = [t["name"] for t in snap["tools"]]
    assert names == ["browse_data_sources", "transform_coordinates"]
