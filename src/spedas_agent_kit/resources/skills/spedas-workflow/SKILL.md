...
artifacts, then use the kept geometry tools (`transform_coordinates`,
`compute_distance`, `get_ephemeris`) for frame/geometry work and an external
PySPEDAS runtime for LMN/FAC or particle-spectra analysis; do not request
removed analysis MCP tools. Keep `*-DIST` artifacts with explicit provenance
for PAD/energy claims. A single-spacecraft `e*n_e*(V_i-V_e)` current or `J·E'` is a
