...
a runtime PySPEDAS script writes figure
...
- For particle products, hand off product/calibration caveats to mission
  skills or an external-runtime particle analysis before interpreting beams,
  crescents, loss cones, or pitch-angle anisotropy.
...
- `transform_coordinates` / `list_coordinate_frames` when plot labels or
  legends depend on frame transforms (`GSE`, `GSM`, `RTN`, `FAC`, `LMN`).
- External-runtime spectral/wave-science interpretation after the figure is
  well defined.
