...
| PWE/OFA wave spectra (chorus/EMIC/hiss/whistler context) | `pyspedas.erg.pwe_ofa(...)` or CDAWeb `ERG_PWE_OFA_L2_SPEC` | `erg_pwe_ofa_l2_spec_E_spectra_*`, `erg_pwe_ofa_l2_spec_B_spectra_*` | E/B spectrogram quick-look; real polarization/wave-normal work needs an external-runtime analysis with explicit provenance |
...
| Orbit / attitude / conjunction context | `pyspedas.erg.orb(...)`, `att(...)`; CDAWeb `ERG_ORB_L2` | `erg_orb_l2_pos_gsm`, attitude/support variables | orbit plot + frame/provenance record; footpoint mapping needs external-runtime work or the SPICE geometry tools (get_ephemeris/compute_distance/transform_coordinates) |
...
Do not create ERG-specific duplicates of existing workflows.

- Chorus/EMIC/whistler polarization, wave-normal angle, ellipticity, and
  degree-of-polarization analysis belongs in an external PySPEDAS runtime with
  artifact-first provenance; no packaged MCP analysis tool provides it.
- Field-aligned pitch-angle, loss-cone, beam, or pancake analysis likewise
  belongs in an external PySPEDAS runtime with provenance.
- 1-D spectra / slopes and coherence / cross-phase analysis belong in an
  external runtime; record the PSD/coherence method and parameters in
  provenance.
- Magnetic footprint or conjugacy work belongs in an external runtime, or in
  the SPICE geometry tools (get_ephemeris, compute_distance,
  transform_coordinates) where kernels are available.
