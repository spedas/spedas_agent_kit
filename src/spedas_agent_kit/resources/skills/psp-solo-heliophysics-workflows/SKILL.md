...
| PSP/SolO radial alignment or conjunction context | Yes | `spice-conjunction-finder` |
| PSP/SolO switchback / Alfvénic impulse reproduction | Yes for route scouting, then specialize | `psp-solar-wind-switchbacks` |
| PSP/SolO turbulence spectrum or PVI event table | Yes for products/cadence, then specialize | `solar-wind-turbulence-intermittency` |
...
5. If geometry matters, load `spice-conjunction-finder` before source mapping or
   radial-alignment claims. Document frames (RTN, VSO,
...
3. Document coordinate frames before comparing vectors (see `transform_coordinates` / `list_coordinate_frames`).
