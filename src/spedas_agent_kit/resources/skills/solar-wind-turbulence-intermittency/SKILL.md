...
energy-transfer / third-order-law context. It complements the
`solar-wind-icme-storm` and `psp-solar-wind-switchbacks` skills for
storm/switchback context; this one covers **intermittency diagnostics and the
provenance needed to avoid over-claiming proxy math**.
...
→ local artifact script for increments/PVI/event table → a saved
matplotlib figure (or a runtime PySPEDAS script) for spectral context.
...
7. **Plot breadcrumbs.** A useful first figure has panels for vector components,
   ...
   markers. For spectral context, compute a PSD with a runtime PySPEDAS script and
   record the method/parameters in provenance instead of reimplementing PSD logic.
