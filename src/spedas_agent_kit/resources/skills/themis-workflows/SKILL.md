...
- `spedas-workflow` for magnetotail boundary, current-sheet, and multi-probe
  route-scouts.
- External-runtime LMN/MVA routines for boundary work after a real boundary
  interval exists; `transform_coordinates` covers frame changes.
- External-runtime wave analysis for THEMIS SCM `scf` / `scp` / `scw`
  valid-window checks.
- External-runtime tail geometry / ionospheric mapping (or the SPICE geometry
  tools where kernels are available) when state/orbit products are needed.
...
- Route validated boundary intervals to external-runtime LMN/MVA routines
  rather than reimplementing LMN/MVA logic here.
...
- Route polarization/spectral analysis to an external runtime (e.g. local
  PySPEDAS `twavpol`); that is an external runtime route, not an MCP tool.
