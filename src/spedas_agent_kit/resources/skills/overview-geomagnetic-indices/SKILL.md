...
- **THEMIS SCM as overview adjunct:** if a THEMIS wave-context paper starts from
  this overview skill, route the waveform/polarization decision to an external-runtime wave
  analysis after checking whether `scf`, `scp`, or `scw` actually has samples in
