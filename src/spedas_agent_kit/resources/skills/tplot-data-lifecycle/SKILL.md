...
- `transform_coordinates` / `list_coordinate_frames` (or external-runtime PySPEDAS routines) when coordinate/frame assumptions matter.
- External-runtime spectral/wave analysis (PSD, polarization, coherence) after cadence and gap checks.
- External-runtime particle-distribution analysis only after particle product shape, units, and calibration caveats are explicit.
