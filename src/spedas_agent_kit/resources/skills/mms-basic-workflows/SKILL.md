...
- External-runtime `pyspedas.projects.mms.curlometer` / `lingradest` / timing
  helpers only after four MMS magnetic-field streams and four position streams
  pass cadence/frame checks.
- External-runtime LMN/MVA/FAC routines for boundary work after a real boundary
  interval exists; `transform_coordinates` covers frame changes.
- External-runtime wave/spectral analysis (SCM/FGM) with explicit provenance;
  do not reimplement polarization logic here.
- External-runtime particle-distribution analysis only when the exact
  FPI/HPCA distribution route, magnetic-field input, cadence, and calibration
  caveats are verified.
...
Before routing to an external `pyspedas.projects.mms.curlometer` /
`lingradest` / timing runtime route, require:
...
- Route polarization/spectral analysis to an external runtime (with
  provenance) after the data window is non-empty and the background field is
  identified.
