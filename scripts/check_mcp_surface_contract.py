...
environment gates are exercised end to end (the legacy CDAWeb/PDS compat tools
are demoted out of the default surface unless ``SPEDAS_AGENT_KIT_COMPAT_TOOLS=1``):

* ``base``   -- gate unset (the default 13-tool surface);
* ``compat`` -- only ``SPEDAS_AGENT_KIT_COMPAT_TOOLS=1``.

The optional ``[analysis]``/``[hapi]``/``[fdsn]`` extras and their tools were
removed in the one-MCP cleanup, so the surface is fully deterministic: no
optional analysis tool names exist to filter. See
``tests/contracts/mcp_surface/README.md``.
...
#: Optional analysis tool names excluded from these profiles. The one-MCP
#: cleanup removed the optional analysis layer, so the exclusion set is empty;
#: kept as a named constant so re-adding optional tools surfaces as a filter
#: change rather than a silent capture change.
...
def build_profile_snapshot(
    ...
    return {
        "profile": profile,
        "env": dict(env_flags),
        # Documented inline so a reviewer reading a snapshot knows no optional
        # analysis tools exist in the current surface. Constant across
        # environments, so it adds no drift noise.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        dest="profiles",
        help="Restrict to these profiles (repeatable). Default: check both.",
    )
    parser.add_argument(
        "--update",
        action="store_true",
    ...
    # Check both profiles in a single invocation by default (one server launch
    # per profile, which is required to exercise each env gate), rather than
    # shelling out twice.
