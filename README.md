...
- `create_spedas_analysis_bundle(study_name, output_dir, ...)` — create a request/provenance scaffold with `requests/`, `data/`, `plots/`, `provenance/run.json`, and `notes/` folders. The seeded `run.json` points to MCP resource `spedas-preset://schemas/analysis_bundle_run`; keep its `tool_calls`, `artifacts`, and `caveats` arrays updated as the workflow progresses. Its `plan_path` and `artifact_dirs` values are normalized, bundle-relative POSIX paths (e.g. `"requests/spedas_plan.json"`, `{"data": "data"}`) so the record stays meaningful after the bundle directory is relocated or copied to another machine; the MCP tool result's own `paths` values remain directly usable absolute filesystem paths. Local Python callers can shape-check the record, including path portability, with `spedas_agent_kit.resources.provenance.validate_analysis_bundle_run(...)`.
