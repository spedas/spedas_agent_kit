...
- `create_spedas_analysis_bundle(study_name, output_dir, ...)` — create a request/provenance scaffold with `requests/`, `data/`, `plots/`, `provenance/run.json`, and `notes/` folders. The seeded `run.json` points to MCP resource `spedas-preset://schemas/analysis_bundle_run`; keep its `tool_calls`, `artifacts`, and `caveats` arrays updated as the workflow progresses. Its `plan_path`, every `artifact_dirs` value, and each `artifacts[].path` value are normalized, bundle-relative POSIX paths (e.g. `"requests/spedas_plan.json"`, `{"data": "data"}`, and `"notes/result.csv"`) so the record stays meaningful after the bundle directory is relocated or copied to another machine; the MCP tool result's own `paths` values remain directly usable absolute filesystem paths. Local Python callers can shape-check the record, including path portability, with `spedas_agent_kit.resources.provenance.validate_analysis_bundle_run(...)`, or verify the recorded plan, artifact directories, and artifact files against a copied bundle root with `spedas_agent_kit.resources.provenance.validate_analysis_bundle_files(...)`. The filesystem check follows symlinks for containment and does not scan unrecorded files.
...
The smoke scripts prefer the checked-out `src/` tree over any ambient installed `spedas_agent_kit` package, so local source-tree runs do not accidentally inspect a stale editable install. The list-tools smoke starts the stdio MCP server with isolated temporary cache directories, performs MCP `initialize` + `list_tools`, and verifies the expected advertised tool names. The resource smoke performs MCP `list_resources` + selected `read_resource` calls for the skill index, workflow skill, and provenance schema resources. The analysis-bundle smoke creates a temporary bundle, reads `spedas-preset://schemas/analysis_bundle_run`, appends one compact `tool_calls` / `artifacts` / `caveats` update to `provenance/run.json`, validates the resulting record shape with the dependency-free `validate_analysis_bundle_run(...)` helper plus optional `jsonschema` when installed, then copies the whole bundle directory (leaving the original in place) into a relocated sibling root and re-validates that the seeded `plan_path` and `artifact_dirs` still resolve correctly under that new, copied bundle root. The analysis smoke imports the exact optional backend modules/attributes and verifies that a real `[analysis]` install registers every analysis tool by default. None of these smokes fetch CDAWeb/PDS data or download SPICE kernels.
