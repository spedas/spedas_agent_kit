...
- **Kernels:** a cold cache returns `needs_confirmation` / `kernel_download_required`; pass `allow_kernel_download=True` or pre-load with `manage_spice_kernels(action='load', mission=...)`.
